...
def portscanner(host, port): # 自定义一个函数
    try: # 尝试运行
...
def main(ip, ports=range(65535)):  # 自定义函数 设置 端口缺省值0-65535，端口不写默认到65535
    setdefaulttimeout(0.1) # 设置超时时间
    for port in ports: # 循环端口
        t = threading.Thread(target=portscanner, args=(ip, port)) # 创建线程
    ...
    with open("dk.txt", "w+") as dk:  # 用with函数，可以防止忘记关闭文件时，所导致的文件读取异常
        ...
        with alive_bar(len(data), force_tty=True) as bar:# 给 alive_bar 传入进度条总数目（这里是 列表data的长度）
            for i in data: # 循环存活靶机，方便每个IP创建线程
                d = threading.Thread(target=main(i.strip(),range(int(ks),int(js)+1))) # 创建线程
