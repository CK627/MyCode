...
from alive_progress import alive_bar # 导入 alive-progress库
import os # 导入内置库os
...
def Survival_host_scanning():
    zt, b, c, d=input("输入开始ip：").split(".") # 定义变量，每个变量用"."隔开
    q,w,e,r=input("输入结束ip：").split(".")
    # data = open('ip.txt', 'a+')  # 打开文件
    for i in range(int(zt), int(q) + 1): # 循环
        for x in range(int(b),int(w)+1):
            for l in range(int(c),int(e)+1):
                for y in range(int(d),int(r)+1):
                    wang=os.popen("fping -t 1 "+str(i)+"."+str(x)+"."+str(l)+"."+str(y)).readlines() # 中心句；调用cmd，读取内容保存到wang这个变量中
                    ze=str(wang).find("alive") # 查找关键词，如歌有则为真（Ture）值大于0
                    if ze>=0: # 如果大于0
                        data.append(str(i)+"."+str(x)+"."+str(l)+"."+str(y)) # 把存活的IP添加到列表中
                    else:
