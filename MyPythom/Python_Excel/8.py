...
import pandas as pd
from PyQt5.QtWidgets import QApplication, QMainWindow, QFileDialog, QListWidget, QPushButton, QLineEdit, QLabel, \
    QTextEdit, QVBoxLayout, QWidget, QCheckBox, QHBoxLayout, QStackedWidget
...
class GUI(QMainWindow):
    def __init__(self):
        ...
        self.list_widget_teacher = None
        self.list_widget_department = None
    ...
    def create_department_page(self, page):
        ...
        self.list_widget_department = self.create_list_widget(department_layout, '选择部门:',
                                                              ["23级动漫制作技术1班","22级药学3班","艺术学院","23级软件技术班","23计算机应用1班","20级园林技术班","23级航一班宋增华","护理学院","建筑工程学院","21信息强化1班",
 ...
 "22级酒店邱萍","机电工程学院","机电工程学院","22医学影像技术5班"])
        ...
        department_select_all_button.clicked.connect(
            lambda: self.select_all_items(self.list_widget_department, department_select_all_button))
        page.setLayout(department_layout)

    def create_teacher_page(self, page):
        teacher_layout = QVBoxLayout()
        self.list_widget_teacher = self.create_list_widget(teacher_layout, '选择导师:', ["全选", "孙泽霖", "孙彤彤"])
        teacher_select_all_button = self.create_select_all_button(teacher_layout, '全选导师')
        teacher_select_all_button.clicked.connect(
            lambda: self.select_all_items(self.list_widget_teacher, teacher_select_all_button))
    ...
    @staticmethod
    def select_all_items(list_widget, select_all_button):
        for index in range(list_widget.count()):
            item = list_widget.item(index)
            item.setSelected(select_all_button.isChecked())

    def processing_college_data(self):
        file_path = self.input_fields[0].text()
        folder_name = self.input_fields[1].text()
        selected_department = self.list_widget_department.selectedItems()
        selected_teacher = self.list_widget_teacher.selectedItems()
        ...
        try:
            ...
            if self.current_mode == 0:
                # Processing based on selected departments
                for item in selected_department:
                    normalized_category = DataProcessor.normalize_department(item.text().split(':')[0])
                    thread = threading.Thread(target=DataProcessor.process_category_data_colleget,
                                              args=(df, normalized_category, output_folder))
                    thread.start()
                    threads.append(thread)
            else:
                # Processing based on selected teachers
                for item in selected_teacher:
                    normalized_category = DataProcessor.normalize_department(item.text().split(':')[0])
            ...
            if self.current_mode == 0:
                ...
                for item in selected_department:
                    normalized_category = DataProcessor.normalize_department(item.text().split(':')[0])
                    category_df = df[df["部门"] == item.text()]
                    row_count = len(category_df)
                    summary_data.append(
                        [DataProcessor.acquisition_time(category_df),
                         normalized_category, row_count])
            else:
                summary_data = []
                for item in selected_teacher:
                    normalized_category = DataProcessor.normalize_department(item.text().split(':')[0])
                    category_df = df[df["导师"] == item.text()]
                    row_count = len(category_df)
                    summary_data.append(
                        [DataProcessor.acquisition_time(category_df),
                         normalized_category, row_count])
