...
def select_file():
    ...
    folder_name_choices = [
        "全选",
        "23级动漫制作技术1班", "22级药学3班", "信息工程学院", "23级软件技术班", "23计算机应用1班", "20级园林技术班",
        "23级航一班宋增华",
...
def process_excel_data_department(file_path, folder_name, selected_departments):
    ...
    try:
        ...
        # 创建一个字典来跟踪个部门和总部门打卡次数
        individual_department_summary = {}
        categorized_summary = {}
        ...
        for category in unique_categories:
            ...
            for key, values in college_mapping.items():
                if any(value in normalized_category for value in values):
            ...
            if not selected_departments or normalized_category in selected_departments:
                ...
                # 更新个部门和总部门打卡次数的字典
                if categorized:
                    if normalized_category in categorized_summary:
                        categorized_summary[normalized_category] += len(df[df["部门"] == category])
                    else:
                        categorized_summary[normalized_category] = len(df[df["部门"] == category])
                else:
                    if normalized_category in individual_department_summary:
                        individual_department_summary[normalized_category] += len(df[df["部门"] == category])
                    else:
                        individual_department_summary[normalized_category] = len(df[df["部门"] == category])

        # 生成个部门的 Excel 文件
        for department, count in individual_department_summary.items():
            department_data = df[df["部门"] == department]
            department_file_path = os.path.join(output_folder, f'{department} 共打卡{count}次.xlsx')
            department_data.to_excel(department_file_path, index=False, engine='openpyxl')

        # 生成归类名的 Excel 文件
        if categorized_summary:
            categorized_summary_data = []
            for category, total_count in categorized_summary.items():
                categorized_summary_data.append([category, total_count])
                categorized_summary_file_path = os.path.join(output_folder, f'{category} 共打卡{total_count}次.xlsx')
                categorized_summary_df = pd.DataFrame(categorized_summary_data, columns=["归类名", "总共次数"])
                categorized_summary_df.to_excel(categorized_summary_file_path, index=False, engine='openpyxl')
...
def process_selected_department_summary(df, selected_departments, output_folder):
    try:
        summary_data = []
        for department in selected_departments:
            department_df = df[df["部门"] == department]
            row_count = len(department_df)
            summary_data.append(
                [acquisition_time(department_df), department, row_count])
        ...
        sg.popup_error(f"生成选定部门统计文件时出错: {str(e)}", title="错误")
